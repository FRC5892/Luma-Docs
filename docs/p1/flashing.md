...
This guide will walk you through flashing the latest PhotonVision image onto your Luma P1 camera.

!!! warning "Read Before You Start"
    Ensure you have completed all steps on the [Prerequisites](prerequisites.md) page before proceeding.

---

## What Is Flashing?

**Flashing** is the process of connecting a device to a computer and using a specialized tool to write an image file (the latest release of PhotonVision) to the device's internal storage. This will overwrite the existing firmware and calibration data on the device.

!!! tip "Don't Want to Lose Calibration?"
    If you are updating a **previously calibrated** Luma to a newer PhotonVision version, use the **Offline Update** feature in PhotonVision instead of flashing the OS. This preserves your calibration settings. *(Offline Update documentation coming soon.)*
...
2. Work through each menu in order. For **every single menu**, scroll to the **bottom** and select the **last option** (no filtering, custom config, etc.).
3. When prompted for a custom image, a file picker will appear—select the **PhotonVision image for Luma P1** that you downloaded earlier.
...
6. A progress bar will appear. Wait for the flash to complete—the imager will notify you when the camera is successfully flashed.
